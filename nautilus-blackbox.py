...
class BlackBoxExtension(GObject.GObject, Nautilus.MenuProvider):
    def _open_terminal(self, file: Nautilus.FileInfo) -> None:
        filename = unquote(file.get_uri()[7:])
        
        subprocess.Popen(["blackbox", "--working-directory", filename])

        # Close the context menu.
        menu.get_parent().cancel() 
    ...
    def get_file_items(
        ...
        item = Nautilus.MenuItem(
            ...
            tip="Open Black Box In %s" % file.get_name(),
        )
    ...
    def get_background_items(
        ...
        item = Nautilus.MenuItem(
            ...
            label="Open in Black Box",
            tip="Open Black Box In %s" % current_folder.get_name(),
        )
